...
cleanup_docker() {
    ...
    # Remove images
    local images=("gemini-mcp-server:latest" "genz-mcp-server:latest")
        ...
        if venv_error=$($python_cmd -m venv "$VENV_PATH" 2>&1); then
            ...
            if [[ "$os_type" == "linux" || "$os_type" == "wsl" ]]; then
                if echo "$venv_error" | grep -E -q "No module named venv|venv.*not found|ensurepip is not|python3.*-venv"; then
                    ...
                    echo "  cd ~ && git clone <repository-url> && cd genz-mcp-server && ./run-server.sh"
                    echo ""
                    exit 1
                else
...
install_dependencies() {
    ...
    echo ""
    print_info "Setting up GenZ MCP Server..."
...
check_claude_cli_integration() {
    ...
    if ! command -v claude &> /dev/null; then
        ...
        echo ""
        read -p "Would you like to add GenZ to Claude Code? (Y/n): " -n 1 -r
        echo ""
        if [[ $REPLY =~ ^[Nn]$ ]]; then
    ...
    # Check if genz is registered
    local mcp_list=$(claude mcp list 2>/dev/null)
    if echo "$mcp_list" | grep -q "genz"; then
        # Check if it's using the old Docker command
        if echo "$mcp_list" | grep -E "genz.*docker|genz.*compose" &>/dev/null; then
            print_warning "Found old Docker-based GenZ registration, updating..."
            claude mcp remove genz -s user 2>/dev/null || true
            
            # Re-add with correct Python command
            if claude mcp add genz -s user -- "$python_cmd" "$server_path" 2>/dev/null; then
                print_success "Updated GenZ to become a standalone script"
                return 0
            else
                echo ""
                echo "Failed to update MCP registration. Please run manually:"
                echo "  claude mcp remove genz -s user"
                echo "  claude mcp add genz -s user -- $python_cmd $server_path"
                return 1
            fi
        else
            ...
            if echo "$mcp_list" | grep -F "$server_path" &>/dev/null; then
                ...
                print_warning "GenZ registered with different path, updating..."
                claude mcp remove genz -s user 2>/dev/null || true
                
                if claude mcp add genz -s user -- "$python_cmd" "$server_path" 2>/dev/null; then
                    print_success "Updated GenZ with current path"
                    return 0
                else
                    echo ""
                    echo "Failed to update MCP registration. Please run manually:"
        ...
        echo ""
        read -p "Add GenZ to Claude Code? (Y/n): " -n 1 -r
        echo ""
        if [[ $REPLY =~ ^[Nn]$ ]]; then
        ...
        print_info "Registering GenZ with Claude Code..."
        if claude mcp add genz -s user -- "$python_cmd" "$server_path" 2>/dev/null; then
            print_success "Successfully added GenZ to Claude Code"
            return 0
        else
            echo ""
...
check_claude_desktop_integration() {
    ...
    echo ""
    read -p "Configure GenZ for Claude Desktop? (Y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Nn]$ ]]; then
    ...
    if [[ -f "$config_path" ]]; then
        ...
        if grep -q "docker.*compose.*genz\|genz.*docker" "$config_path" 2>/dev/null; then
            print_warning "Removing old Docker-based MCP configuration..."
            # Create backup
            cp "$config_path" "${config_path}.backup_$(date +%Y%m%d_%H%M%S)"
            
            # Remove old genz config using a more robust approach
...
try:
    ...
    # Remove genz from mcpServers if it exists
    if 'mcpServers' in config and 'genz' in config['mcpServers']:
        del config['mcpServers']['genz']
        print('Removed old genz MCP configuration')
...
check_gemini_cli_integration() {
    local script_dir="$1"
    local genz_wrapper="$script_dir/genz-mcp-server"
    ...
    # Check if genz is already configured
    if grep -q '"genz"' "$gemini_config" 2>/dev/null; then
        # Already configured
        return 0
    fi
    
    # Ask user if they want to add GenZ to Gemini CLI
    echo ""
    read -p "Configure GenZ for Gemini CLI? (Y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Nn]$ ]]; then
    ...
    if [[ ! -f "$genz_wrapper" ]]; then
        print_info "Creating wrapper script for Gemini CLI..."
        cat > "$genz_wrapper" << 'EOF'
#!/bin/bash
# Wrapper script for Gemini CLI compatibility
DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$DIR"
exec .genz_venv/bin/python server.py "$@"
EOF
        chmod +x "$genz_wrapper"
        print_success "Created genz-mcp-server wrapper script"
    fi
    ...
    # Add genz configuration using Python for proper JSON handling
    local temp_file=$(mktemp)
    python3 -c "
import json
import sys
...
" && mv "$temp_file" "$gemini_config"
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo "  Restart Gemini CLI to use GenZ MCP Server"
    else
...
display_config_instructions() {
    ...
    echo ""
    local config_header="GENZ MCP SERVER CONFIGURATION"
    echo "===== $config_header ====="
    printf '%*s\n' "$((${#config_header} + 12))" | tr ' ' '='
    echo ""
    echo "To use GenZ MCP Server with your Claude clients:"
    echo ""
    
    print_info "1. For Claude Code (CLI):"
    echo -e "   ${GREEN}claude mcp add genz -s user -- $python_cmd $server_path${NC}"
    echo ""
...
EOF
    ...
    cat << EOF
   {
     "mcpServers": {
       "genz": {
         "command": "$script_dir/genz-mcp-server"
       }
     }
   }
EOF
...
display_setup_instructions() {
    ...
    printf '%*s\n' "$((${#setup_header} + 12))" | tr ' ' '='
    echo ""
    print_success "GenZ is ready to use!"
}
...
show_help() {
    ...
    local header="🤖 GenZ MCP Server v$version"
    echo "$header"
    printf '%*s\n' "${#header}" | tr ' ' '='
    echo ""
    ...
    echo "  https://github.com/petems/genz-mcp-server"
}
...
main() {
    ...
    # Display header
    local main_header="🤖 GenZ MCP Server"
    echo "$main_header"
    printf '%*s\n' "${#main_header}" | tr ' ' '='
