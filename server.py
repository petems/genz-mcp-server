"""
GenZ MCP Server - Main server implementation

This module implements the core MCP (Model Context Protocol) server that provides
AI-powered tools for chat and debug assistance using multiple AI models.
...
for chat and debugging capabilities.
...
server: Server = Server("genz-server")


# Constants for tool filtering
ESSENTIAL_TOOLS = set()  # No essential tools in GenZ version
...
# Initialize the tool registry with available GenZ tools
# Each tool provides specialized functionality for different development tasks
# Tools are instantiated once and reused across requests (stateless design)
TOOLS = {
    "genz_chat": GenZChatTool(),  # Interactive development chat and brainstorming
    "genz_debug": GenZDebugIssueTool(),  # Root cause analysis and debugging assistance
}
TOOLS = filter_disabled_tools(TOOLS)

# Rich prompt templates for GenZ tools
PROMPT_TEMPLATES = {
    "genz_chat": {
        "name": "genz_chat",
...
async def handle_get_prompt(name: str, arguments: dict[str, Any] = None) -> GetPromptResult:
    ...
    if name.lower() == "continue":
        # This is "/genz:continue" - use chat tool as default for continuation
        tool_name = "genz_chat"
        template_info = {
            "name": "continue",
        ...
        logger.debug("Using /genz:continue - defaulting to genz_chat tool")
    else:
...
async def main():
    ...
    # Log startup message
    logger.info("GenZ MCP Server starting up...")
...
def run():
    """Console script entry point for genz-mcp-server."""
    try:
        asyncio.run(main())
